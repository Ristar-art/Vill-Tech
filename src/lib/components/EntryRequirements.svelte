<script>
  ...
  $: entryRequirements = courseCompetencies?.find(comp => comp.competency.shortname === 'Entry Requirements');
...
<div class="bg-white mt-6 p-6 rounded-lg shadow-md border border-gray-200">
  <div class="flex items-start gap-4">
    ...
    <div class="flex-grow">
      <h3 class="text-xl font-semibold text-[#222222] mb-2">{entryRequirements?.competency?.shortname || 'Entry Requirements'}</h3>
      <div class="text-gray-700">
        <div 
          class="prose prose-sm max-w-none [&_ul]:space-y-3 [&_li]:flex [&_li]:items-start [&_p]:m-0"
        >
          {@html cleanDescription(entryRequirements?.competency?.description)}
        </div>
      </div>
    </div>
  </div>
