<script>
  ...
  $: module4 = courseCompetencies?.find(comp => comp.competency.shortname === 'MODULE 4');
...
<div class="bg-white bg-opacity-90 rounded-tl-[40px] rounded-br-[40px] p-8 shadow-xl mb-8" >
    ...
    {module4?.competency?.shortname || 'MODULE 4'}
  </h3>

  {#if module4?.competency?.description}
    {@const content = cleanDescription(module4.competency.description)}
