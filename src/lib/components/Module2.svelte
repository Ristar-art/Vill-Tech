<script>
  ...
  $: module2 = courseCompetencies?.find(comp => comp.competency.shortname === 'MODULE 2');
...
<div class="bg-gray-50 p-6 rounded-lg shadow-md border border-gray-200 mt-6">
  ...
  {#if module2?.competency?.description}
    {@const content = cleanDescription(module2.competency.description)}
