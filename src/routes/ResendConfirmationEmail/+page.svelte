<script>
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await moodleClient.resendConfirmationEmail(formData.username, formData.password);
            if (response.status === 1) {
                submitStatus = 'success';
                formData = { username: '', password: '' }; // Clear form data
            } else {
                error = response.warnings && response.warnings[0] ? response.warnings[0].message : 'Failed to resend confirmation email';
            }
        } catch (err) {
            console.error(err);
            error = err instanceof Error ? err.message : 'An error occurred during resend confirmation email request';
        } finally {
            loading = false;
            isSubmitting = false;
        }
...
<svelte:head>
    <title>Resend Confirmation Email - Village Tech</title>
    <meta name="description" content="Resend confirmation email for your Village Tech Training Solutions account" />
</svelte:head>
            ...
            <div class="space-y-6">
                ...
                <div class="text-center space-y-2">
                    <h1 class="text-3xl font-bold">
                        ...
                        <span class="text-red-500">Confirmation Email</span>
                    </h1>
                    <p class="text-gray-500">
                        Enter your username and password to resend the confirmation email
                    </p>
                </div>
                ...
                <form on:submit={handleSubmit} class="space-y-4">
                    ...
                    {#if submitStatus === 'success'}
                        <div class="text-green-600 text-center flex items-center justify-center" in:fade>
                            ...
                            Confirmation email sent successfully! Check your email.
                        </div>
                    {/if}
