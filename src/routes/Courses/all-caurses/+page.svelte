<script lang="ts">
    ...
    export let filter = "all"; // Bindable filter prop
  
    // Filter function based on course category
    function filterCourses(courses: any[], filter: string) {
      if (!courses || filter === "all") return courses;
      return courses.filter((course) => course.category?.toLowerCase() === filter.toLowerCase());
    }
  
    $: filteredCourses = filterCourses(loadedCourses, filter); // Reactive filtered list
  </script>
  ...
  <div class="min-h-screen bg-[#21409a] ">
    <div class="h-[60vh] flex flex-col items-center justify-center pt-20 text-center bg-cover bg-center" style="background-image: url('https://picsum.photos/500/300?random=9');">
